...
$DOCKER_PREFIX docker run --platform linux/arm64 -v $OUTDIR:/out -v $BPATH/build-linux.sh:/bin/build.sh -w /build --rm -it $BUILD_IMAGE bash /bin/build.sh arm64 || true
$DOCKER_PREFIX docker run --platform linux/arm -v $OUTDIR:/out -v $BPATH/build-linux.sh:/bin/build.sh -w /build --rm -it $BUILD_IMAGE bash /bin/build.sh arm || true
$DOCKER_PREFIX docker run --platform linux/i386 -v $OUTDIR:/out -v $BPATH/build-linux.sh:/bin/build.sh -w /build --rm -it $BUILD_IMAGE bash /bin/build.sh i386 || true
